...
A recording/file ID is the stable Meeting identity when Plaud supplies it. Otherwise, PPA derives the identity solely from normalized Create Time; title, transcript, summary, and participants do not affect it. The delivery event ID is the separate event idempotency key. Transcript and summary events merge safely into one provider-neutral Meeting. Phase 1.3A never starts Meeting Intelligence automatically. Future CLI backfill must submit the same normalized contract through PPA rather than writing the database directly.
