...
The n8n mapping accepts common Zapier spellings such as `recording_id`, `recordingId`, `file_id`, `event_id`, `eventId`, `transcript`, and `summary`. A supplied Zapier/Plaud event ID is preferred. If absent, n8n deterministically derives one from the recording ID, normalized trigger type, a content/version timestamp when present, and content. It never uses a random UUID, so retries remain idempotent. Inspect the n8n execution's **Input** and **Output** panels for a test event's field *names*; update only this mapping after observing an actual Zapier sample. Avoid retaining full transcripts in debug logs.
...
3. Create a Zap with either **Transcript generated** or **Summary generated** as the Plaud trigger.
